...
def get_feature_categ_corrs(subject, feature_type, which_prf_grid=1, layer_name=None):
    ...
    labels_categ, categ_names = label_utils.load_highlevel_categ_labels_each_prf(subject, \
                         which_prf_grid, image_inds=image_inds, models=models)
   
    axes_use = [0,1,2,3,4,5,6,7]
    labels_all = labels_categ[:,axes_use,:]
    categ_names = np.array(categ_names)[axes_use]
    print('using axes:')
    print(categ_names)
    unique_labels_each = [np.unique(labels_categ[:,ax]) for ax in range(labels_categ.shape[1])]
    ...
    fn2save_corrs = os.path.join(path_to_save, '%s_%s_categ_corrs_grid%d.npy'\
                                 %(substr, feature_type, which_prf_grid))
    fn2save_discrim = os.path.join(path_to_save, '%s_%s_categ_discrim_tstat_grid%d.npy'\
                                 %(substr, feature_type, which_prf_grid))
    fn2save_nsamp = os.path.join(path_to_save, '%s_%s_categ_nsamp_grid%d.npy'\
                                 %(substr, feature_type, which_prf_grid))
    fn2save_partial_corrs = os.path.join(path_to_save, '%s_%s_categ_partial_corrs_grid%d.npy'\
                                 %(substr, feature_type, which_prf_grid))
    fn2save_nsamp_partial = os.path.join(path_to_save, '%s_%s_categ_nsamp_partial_corrs_grid%d.npy'\
                                 %(substr, feature_type, which_prf_grid))
    ...
    for prf_model_index in range(n_prfs):
        ...
        for aa in range(n_sem_axes):
            ...
            if prf_model_index==0:
                print('processing axis: %s'%categ_names[aa])
                print('labels: ')
...
if __name__ == '__main__':
    ...
    get_feature_categ_corrs(subject=args.subject, feature_type=args.feature_type, which_prf_grid=args.which_prf_grid, layer_name=args.layer_name)
