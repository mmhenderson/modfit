...
def fit_gabor_fwrf(subject=1, roi='V1', up_to_sess=1, n_ori=36, n_sf=12, sample_batch_size=50, voxel_batch_size=100, zscore_features=True, nonlin_fn=False, debug=False):
...
#     lambdas = np.logspace(-6., 1., 9).astype(np.float64)
    ...
    fn2save = output_dir+'model_params_fakerandomdata'
...
if __name__ == '__main__':
    ...
    fit_gabor_fwrf(int(subj), roi, int(up_to_sess), int(n_ori), int(n_sf), int(sample_batch_size), int(voxel_batch_size), zscore_features, nonlin_fn, debug)