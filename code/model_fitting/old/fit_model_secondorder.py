...
def fit_gabor_fwrf(subject=1, roi='V1', up_to_sess=1, n_ori=36, n_sf=12, sample_batch_size=50, voxel_batch_size=100, zscore_features=True, nonlin_fn=False, padding_mode = 'circular', debug=False):
...
#     lambdas = np.array([0.0,0.0,0.0])
    ...
    best_losses, best_lambdas, best_params, covar_each_model_training, second_order_zstats = learn_params_second_order(
    ...
    for mm in range(n_prfs):
        ...
        features = get_features_in_prf_second_order(models[mm,:], _fmaps_fn, val_stim_single_trial_data, sample_batch_size, aperture, device, zstats=second_order_zstats[mm,:])     
    ...
    fn2save = output_dir+'model_params_secondorder_%s'%roi2print
    ...
    'second_order_zstats': second_order_zstats,
    'nonlin_fn': nonlin_fn,
...
if __name__ == '__main__':
    ...
    if len(sys.argv)>10:
        padding_mode=str(sys.argv[10])
    else:
        padding_mode = 'circular'
    print('padding mode is %s'%padding_mode)   
    ...
    fit_gabor_fwrf(int(subj), roi, int(up_to_sess), int(n_ori), int(n_sf), int(sample_batch_size), int(voxel_batch_size), zscore_features, nonlin_fn, padding_mode, debug)