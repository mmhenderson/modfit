...
from utils import numpy_utils, torch_utils, stats_utils, default_paths, prf_utils
...
def get_population_recons(best_params, prf_models, voxel_data, roi_def, images, _feature_extractor, \
    ...
    feature_inds = np.ones((n_features,))==1
    pop_recons = np.zeros((n_trials, n_features, map_res, map_res, n_rois),dtype=dtype)
    ...
    with torch.no_grad(): # make sure local gradients are off to save memory
        ...
        for rr in range(n_rois):
            ...
            recon_pop = np.zeros((n_trials, n_features, map_res, map_res))
            for vi, vv in enumerate(inds_this_roi):

                # reconstruction is [features x trials]
                recon_voxel = np.linalg.pinv(W[vi:vi+1,:]) @ V[:,vi:vi+1].T
                recon_voxel = recon_voxel.T
                
                x,y,sigma = prf_models[best_model_inds[vv,0],:]
                prf = prf_utils.gauss_2d(center=[x,y], sd=sigma, patch_size=map_res)
    
                prf_tiled = torch.Tensor(prf).view([1,1, map_res, map_res]).expand([n_trials, n_features, \
                                                                                    map_res,map_res])
                recon_tiled = torch.Tensor(recon).view([n_trials, n_features,1,1]).expand([n_trials, n_features, \
                                                                           map_res,map_res])

                recon_pop += recon_tiled * prf_tiled
