...
def fit_bdcn_model(images, voxel_data, _feature_extractor, models, lambdas, do_pca=True, min_pct_var = 99, max_pc_to_retain=100, \
    ...
    for mm in range(len(models)):
        bbox = texture_utils.get_bbox_from_prf(models[mm], image_size, n_prf_sd_out = _feature_extractor.n_prf_sd_out, min_pix=None, \
...
#                 trn_scores = pca.transform(trn_features)
            ...
            for rv,lv in numpy_utility.iterate_range(0, n_voxels, voxel_batch_size):
                ...
                if np.sum(imp)>0:
                    ...
                    best_w_params[arv,0:n_features_actual] = numpy_utility.select_along_axis(betas[:,0:n_features_actual,imp], li, run_axis=2, choice_axis=0).T
