            ...
            for pp in range(n_partial_versions):
                ...
                for rv,lv in numpy_utils.iterate_range(0, n_voxels, voxel_batch_size):
                    ...
                    if np.sum(imp)>0:                    
                        ...
                        best_w_tmp[arv,:] = numpy_utils.select_along_axis(betas[:,:,imp], li, run_axis=2, choice_axis=0).T
...
def fit_bdcn_model(images, voxel_data, _feature_extractor, models, lambdas, do_pca=True, min_pct_var = 99, max_pc_to_retain=100, \
                       aperture=1.0, zscore=False, voxel_batch_size=100, holdout_size=100, \
                       shuffle=True, shuff_rnd_seed=0, add_bias=False, device=None, debug=False):
    
    """
    Learn parameters of encoding model with BDCN edge maps as features.
    Can perform PCA first or skip.
    """
    ...
    downsampled_size = np.ceil(image_size/_feature_extractor.downsample_factor).astype('int')
    n_features = np.zeros(shape=(len(models),),dtype=int)
    for mm in range(len(models)):
        bbox = texture_utils.get_bbox_from_prf(models[mm], downsampled_size, n_prf_sd_out = _feature_extractor.n_prf_sd_out, min_pix=None, \
    ...
    if do_pca:        
        ...
        pca_wts = [np.zeros(shape=(max_pc_to_retain, n_features[mm]), dtype=dtype) for mm in range(n_prfs)] 
        pca_pre_mean = [np.zeros(shape=(n_features[mm],), dtype=dtype) for mm in range(n_prfs)]
#         pca_wts = np.zeros(shape=(max_pc_to_retain, max_features, n_prfs), dtype=dtype) # will be [ncomponents x nfeatures x nmodels]
#         pca_pre_mean = np.zeros(shape=(max_features, n_prfs), dtype=dtype)
        pct_var_expl = np.zeros(shape=(max_pc_to_retain, n_prfs), dtype=dtype)
        n_comp_needed = np.zeros(shape=(n_prfs), dtype=np.int)

        best_w_params = np.zeros(shape=(n_voxels, max_pc_to_retain), dtype=dtype)

    else:
        best_w_params = np.zeros(shape=(n_voxels, max_features), dtype=dtype)
    ...
    if zscore:
        if do_pca:
            features_mean = np.zeros(shape=(n_voxels, max_pc_to_retain), dtype=dtype)
            features_std  = np.zeros(shape=(n_voxels, max_pc_to_retain), dtype=dtype)
    ...
    with torch.no_grad():
        ...
        for m,(x,y,sigma) in enumerate(models):
            ...
            t = time.time()            
            # Get features for the desired pRF, across all trn set image            
            features = _feature_extractor(images, (x,y,sigma)).detach().cpu().numpy()   
            elapsed = time.time() - t

            # separate design matrix into training/held out data (for lambda selection)
            trn_features = features[:trn_size]
            out_features = features[trn_size:]   
            n_features_actual = trn_features.shape[1]
            assert(n_features_actual == n_features[m])
            print('Original dims of training features:')
            ...
            if do_pca:
                ...
                pca = decomposition.PCA(n_components = np.min([np.min([max_pc_to_retain, n_features_actual]), trn_size]), copy=False)
                # Perform PCA to decorrelate feats and reduce dimensionality
                trn_scores = pca.fit_transform(trn_features)
                print('removing old trn features from memory')
                trn_features = None
#                 trn_scores = pca.transform(trn_features)
                print('Training data has been fit, transforming held-out data')
    
                out_scores = pca.transform(out_features)
                print('removing old heldout features from memory')
                out_features = None
                print('extracting weights from pca structure')
                wts = pca.components_
                print('extracting ev from pca structure')
                ...
                print('determining ncomp')
                n_components_actual = np.min([wts.shape[0], max_pc_to_retain])
                # save a record of the transformation to be used for validating model
                print('putting wts into big matrix')
                pca_wts[m][0:n_components_actual,0:n_features_actual] = wts[0:n_components_actual,:] 
                # mean of each feature, nfeatures long - needed to reproduce transformation
                print('putting pca mean into big matrix')
                pca_pre_mean[m][0:n_features_actual] = pca.mean_ 
                # max len of ev is the number of components
                print('putting var expl into big matrix')
                pct_var_expl[0:n_components_actual,m] = ev[0:n_components_actual]  
                print('computing ncomp reduced to use')
                n_components_reduced = int(np.where(np.cumsum(ev)>min_pct_var)[0][0] if np.any(np.cumsum(ev)>min_pct_var) else len(ev))
                
                n_comp_needed[m] = n_components_reduced

                print('Retaining %d components to expl %d pct var\n'%(n_components_reduced, min_pct_var))
                
                assert(n_components_reduced<=max_pc_to_retain)
                trn_features = trn_scores[:,0:n_components_reduced]
                out_features = out_scores[:,0:n_components_reduced]
 
                n_features_actual = n_components_reduced
            ...
            for rv,lv in numpy_utils.iterate_range(0, n_voxels, voxel_batch_size):
                ...
                if np.sum(imp)>0:
                    ...
                    best_models[arv] = m
                    if zscore:
                        features_mean[arv,0:n_features_actual] = features_m # broadcast over updated voxels
                        features_std[arv,0:n_features_actual]  = features_s
                        features_mean[arv,n_features_actual:] = 0.0 # make sure to fill zeros here
                        features_std[arv,n_features_actual:] = 0.0 # make sure to fill zeros here
                    # taking the weights associated with the best lambda value
                    # remember that they won't fill entire matrix, rest of values stay at zero
                    best_w_params[arv,0:n_features_actual] = numpy_utils.select_along_axis(betas[:,0:n_features_actual,imp], li, run_axis=2, choice_axis=0).T
                    best_w_params[arv,n_features_actual:] = 0.0 # make sure to fill zeros here
                    # bias is always last value, even if zeros for the later features
                    if add_bias:
                        best_w_params[arv,-1] = numpy_utils.select_along_axis(betas[:,-1,imp], li, run_axis=1, choice_axis=0).T
    ...
    if do_pca:
        pc = [pca_wts, pct_var_expl, min_pct_var, n_comp_needed, pca_pre_mean]
    else:
        pc = None
        
    print('about to return from fitting')
    sys.stdout.flush()
    
    
    return best_losses, best_lambdas, best_params, pc
