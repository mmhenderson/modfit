...
import utils.numpy_utility as pnu
from utils.numpy_utility import iterate_range
...
class Torch_fwRF_voxel_block(nn.Module):
    ...
    def load_voxel_block(self, *params):
        ...
        if self.combs_zstats is not None:
            ...
            set_value(self.combs_zstats_this_batch, self.combs_zstats[best_model_inds,:])
...
#             print([self.pca_wts_this_batch.shape[0],len(best_model_inds)])
    ...
    def forward(self, _fmaps):
        ...
        if self.combs_zstats is not None:    
            ...
            f = fwrf_fit._to_torch(np.zeros(shape=(_features.shape[0], _features.shape[1]*_features.shape[1]+_features.shape[1], _features.shape[2]),dtype=self.combs_zstats.dtype), device=_features.device)
...
#         print(_features.shape)
        ...
        if self.pca_wts is not None:            
            ...
            features_pca = fwrf_fit._to_torch(np.zeros(shape=_features.shape, dtype=self.pca_wts.dtype), device=_features.device)
    ...
    with torch.no_grad():
        ...
        for rv, lv in iterate_range(0, n_voxels, voxel_batch_size):
            ...
            for rt, lt in iterate_range(0, n_trials, sample_batch_size):
                ...
                pred_block[rt] = fwrf_fit.get_value(_fwrf_fn(_fmaps_fn(fwrf_fit._to_torch(images[rt], device)))) 
