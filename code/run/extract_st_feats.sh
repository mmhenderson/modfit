...
do
    ...
    matlab -nodisplay -nodesktop -nosplash -r "get_st_features_wrapper($image_set,$image_filename,$save_dir,$sketchtokens_dir,$toolbox_dir,$batch_size,$debug); exit"

    cd ${code_path}
    
    python3 extract_sketch_token_features.py --subject $subject --use_node_storage $use_node_storage --debug $debug --which_prf_grid $which_prf_grid --batch_size $batch_size
