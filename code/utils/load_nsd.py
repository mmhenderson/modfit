...
def load_beta_file(filename, voxel_mask=None, zscore=True):
    from utils.file_utility import load_mask_from_nii
    if ".mat" in filename:
...
def load_betas(folder_name, zscore=False, voxel_mask=None, up_to=0, load_ext='.mat'):
    ...
    from utils.file_utility import list_files
    
    matfiles, betas = [], []
    k = 0
    print(folder_name)
    print(len(list_files(folder_name)))
...
def data_split(stim, voxel, ordering, imagewise=True):
    ...
    # the first 1000 images of the subject-specific image matrix (which is 10000 images)
    # are shared indices. So any trials that have those images will be "validation" set.
    shared_mask = ordering_data<1000  

    val_voxel_st = voxel[shared_mask]    # voxels are already ordered by trial numbers
    # images in "stim" are not yet ordered by trial number, so doing this now. This also might mean making duplicates of some images etc.
    ...
    # Averaging over repetition of the same stimulus - should be up to 3 per expt. 
    # Might be fewer if looking at just first few sessions.
    ...
    if imagewise:
        ...
        # for training data, returning individual trials and not averaged over repetitions.
        # for validation data, returning both individual and averaged.
