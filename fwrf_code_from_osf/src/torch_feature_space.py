...
#    _fmaps = _fmaps_fn(_to_torch(data[:batch_size], device=device))
    ...
    # keeping a running total of mean and mean of squares within each group of feature maps and each map within that group.
    # for gabor model: [nResolutions x nOrientations]
    # The resolutions go from lowest SF (smallest feature maps size) to highest SF here.
    ...
    for k in range(len(fb)):
        ...
        run_sqr[k] /= size
    ### 
    # Based on calculations above, looping over groups of maps again and computing variance
    # Using variance to select most informative maps per group.
    # if not that many features in the layer, keeping all true. 
    ...
    if concatenate:
        # Wanting to loop over these from largest (high SF) to smallest (low SF) now
        for k,us in enumerate(np.unique([np.prod(fs[2:4]) for fs in fmaps_sizes])[::-1]): ## they appear sorted from small to large, so I reversed the order
            # mask over layers that have that spatial size (note this is in the original order which is reversed)
            mask = np.array([np.prod(fs[2:4])==us for fs in fmaps_sizes]) 
            # list of index for layers that have that size
            lmask = np.arange(len(fmaps_sizes))[mask] 
            bfmask = np.concatenate([fmask[l] for l in lmask], axis=0)
            
            # clmask is a list nGroups long, going in order of largest feature map group to smallest.
            # each element tells which group in the original feature map ordering corresponds to that group in new ordering.
            clmask += [lmask,]
            # cfmask is list in same order, but each element contains a mask for which feature maps in that group had the 
            # most variance (probably all trues)
            cfmask += [np.arange(len(bfmask))[bfmask],]
            # cfmaps only used to get sizes, not really needed
    ...
    ###
    # tuning masks is [nGroups x nOrient*nResolutions]
    # describes which of the full features list (nOrient*nSF) correspond to each layer group.
    # this is still in the original ordering, not reversed.
    # the mapping from feature map groups to the full feature list is actually implemented in learn_params_ridge_regression
    # and goes in the normal order.
    ...
    # the first item returned is a module that applies whatever filtering we've computed here.
    # in the gabor case this seems to only reverse the order of the layers (e.g. the resolutions)
