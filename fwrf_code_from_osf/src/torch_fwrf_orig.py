...
def learn_params_ridge_regression(data, voxels, _fmaps_fn, models, lambdas, aperture=1.0, _nonlinearity=None, zscore=False, sample_batch_size=100, voxel_batch_size=100, holdout_size=100, shuffle=True, add_bias=False, debug=False):
    """
    ...
    ### Looping over the feature maps once with a batch of data, to get their sizes. ####
    nf = 0
...
#             print('\nStarting model %d of %d - making features'%(m, len(models)))
            ...
            for rt,rl in iterate_range(0, nt, sample_batch_size):
                ...
                # note this is concatenating SFs together from low to high - cycles through all orient channels in order for first SF, then again for next SF.
